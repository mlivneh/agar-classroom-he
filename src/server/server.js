...
const addPlayer = (socket) => {
    ...
    socket.on('gotit', function (clientPlayerData) {
        ...
        if (map.players.findIndexByID(socket.id) > -1) {
            ...
            socket.emit('kick', 'כינוי לא תקין.');
            socket.disconnect();
        } else {
    ...
    // דופק חיבור — בלתי תלוי בתנועה (אירוע "0" = יעד עכבר/מקלדת בלבד)
    socket.on('heartbeat', () => {
        currentPlayer.setLastHeartbeat();
    });
...
const tickPlayer = (currentPlayer) => {
    if (currentPlayer.lastHeartbeat < new Date().getTime() - config.maxHeartbeatInterval) {
        sockets[currentPlayer.id].emit('kick', 'לא התקבל דופק מהלקוח במשך ' + (config.maxHeartbeatInterval / 1000) + ' שניות (טאב ברקע או תקשורת איטית).');
