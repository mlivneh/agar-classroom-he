...
describe('util.js', () => {
  ...
  describe('validNick', () => {
    ...
    it('should allow Hebrew and other Unicode letter nicknames', () => {
      expect(util.validNick('דני')).to.be.true;
      expect(util.validNick('מורה1')).to.be.true;
      expect(util.validNick('Йèæü')).to.be.true;
    });

    it('should disallow spaces and punctuation in nicknames', () => {
      expect(util.validNick('Walter White')).to.be.false;
      expect(util.validNick('a!b')).to.be.false;
      expect(util.validNick('שם עם רווח')).to.be.false;
    });
  });
